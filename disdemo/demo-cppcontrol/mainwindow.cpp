...
// 将 WGS84 航向转换为墨卡托航向​​
double wgs84BearingToMercator(double bearing, double lon, double lat, double distance = 1) {
	double R = 6371000; // 地球半径（米）
	double α = bearing * (M_PI / 180); // 弧度

	// 1. 沿 WGS84 航向计算目标点
	double lat2 = lat + (distance / R) * qCos(α) * (180 / M_PI);
	double lon2 = lon + (distance / R) * qSin(α) / qCos(lat * M_PI / 180) * (180 / M_PI);
...
void MainWindow::processEntityState(const DIS::Pdu& packet)
{
	...
	auto mercatorBearing = wgs84BearingToMercator(wgs84bearing, wgs84llh.longitude, wgs84llh.latitude);
